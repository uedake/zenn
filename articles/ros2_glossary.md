...
| launchファイル | プログラムを決まった手順で立ち上げるための処理を記述したファイル。基本的にはロボットシステムを構成するexecutableを起動するために使用するが、任意のプログラムの起動にも使用できる。launchファイルを用意すると`ros2 launch`コマンドでlaunchファイルに記述した処理を実行できるようになる。 | python<br/>yaml<br/>xml |
...
| 用語 | 意味 | 解説記事 |
| ---- | ---- | ---- |
| ROSノード | ROS1やROS2上で実行する処理を提供する最も基本的なオブジェクト（論理処理単位）。ROSノードクラスをインスタンス化して生成する | [コンセプト](https://zenn.dev/uedake/articles/ros2_concept1_basic) |
| ROSノードクラス | ROSノードを生成する為のクラス実装でありクライアント言語（c++やpython等）で実装される。標準ROSノードクラスとカスタムROSノードクラスに分かれる | [コンセプト](https://zenn.dev/uedake/articles/ros2_concept1_basic) |
| 標準ROSノードクラス | `Node`クラス及び`LifecycleNode`クラスのこと | [コンセプト](https://zenn.dev/uedake/articles/ros2_concept1_basic) |
| カスタムROSノードクラス | 独自に実装した`Node`派生クラスもしくは`LifecycleNode`派生クラスのこと | [コンセプト](https://zenn.dev/uedake/articles/ros2_concept1_basic) |
...
| 用語 | 意味 | 解説記事 |
| ---- | ---- | ---- |
| ROS引数 | ROSノードの初期化を制御する為の値。ノードパラメータの初期値の設定やremapルールの指定ができる。同じexecutableで生成される全ノードで共通のグローバルROS引数（=コマンドラインROS引数）と、各ノード毎のローカルROS引数の２種類がある。グローバルROS引数は、`ros2 run`コマンドで`--ros-args`と記載した後に指定するコマンドラインROS引数から生成される。 | [コンセプト](https://zenn.dev/uedake/articles/ros2_concept3_parameters) |
