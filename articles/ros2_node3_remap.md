...
本記事の目標は、remapで行われている置換処理の詳細を理解することで、完全修飾ノード名が衝突しないようなremapを記述するにはどのようにすれば良いかを理解することです。
...
でも、ノードをconstructする時には自由にノード名とノード名前空間を指定できます。であれば「ノード名とノード名前空間をノード生成時に書き換える」なんて処理は一見不要にも思えます。
