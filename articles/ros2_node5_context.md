...
- ROS2では、ROS引数が実行時のノードの初期化において重要な働きをします。
- ROS引数は、ノード毎のローカルROS引数、executable毎のグローバルROS引数の２種類がありますが、まず理解が大事なのはグローバルROS引数です。グローバルROS引数はコマンドラインROS引数（executableを起動する時の引数として、`--ros-args`と記載した後に指定する所定のオプション）から生成されます
