...
| ---- | ---- | ---- | ---- |
| トピック | 〇：パブリッシュサブスクライブ（メモリ渡し） | 〇：パブリッシュサブスクライブ（トピック通信） | - |
| サービス | 〇：リクエストレスポンス（メモリ渡し） | 〇：リクエストレスポンス（サービス通信） | - |
| アクション | 〇：ゴール設定/結果取得/フィードバック（メモリ渡し） | 〇：ゴール設定/結果取得（サービス通信）、フィードバック（トピック通信） | - |
| ノードパラメータ | 〇：リードライト（メモリ渡し） | 〇：リードライト（サービス通信） | 〇：初期値設定（ROS引数） |
| command interface / state interface | 〇：リードライト（メモリ渡し） | - | - |

command interface / state interface以外は、全て通信に対応しており、異なるプロセス上・異なるマシン上のノード間で情報をやりとりできるのがポイントです。command interfaceやstate interfaceは、ros2_contorolで使用するIFでありプロセス内の情報IFのみを持ちます（pluginによる結合です）

また、自プロセス内のノードとのIFは、通信とはならずメモリ渡しになります

- 注：componentとして実装して、executer上で実行した場合には自動的にメモリ渡しとなる様子だが、componentを用いない場合は未確認

初歩的には１ノード=１プロセスとして、プロセスを分けてノードを実行することが多いですが、複数ノードを１プロセスで動作させることで効率的な動作にすることが可能です（プロセス間通信のようなエンコード・TCP通信・デコードが発生しない）。
...
- ノードに外部から入ってくる情報
  - 能動的
    ...
    - 他ノードのサービスを実行したレスポンスを得る
    - 他ノードのアクションを実行した結果を得る
  - 受動的
    - サブスクライブしているトピックからメッセージを得る
    - executable起動時にROS引数によってノードパラメータの初期値を設定される
    - 他ノードからノードパラメータをライトされ値を書き換えられる
    - 他ノードからのサービスをリクエストされ引数を受け取る
    - 他ノードからのアクションのゴール設定が呼ばれ引数を受け取る
    - 他ノードのアクションを実行したフィードバックと結果を得る
- ノードから外部に出ていく情報
  - 能動的
    - トピックへメッセージをパブリッシュする
    - 他ノードのノードパラメータを書き換える
    - 他ノードのサービスをリクエストするときに引数を渡す
    - 他ノードのアクションをリクエストするとき引数を渡す
  - 受動的
    - 他ノードからノードパラメータをリードされ値を返す
    - 他ノードや外部プロセスからノードが持つサービスが実行されレスポンスを返す
    - 他ノードや外部プロセスからノードが持つアクションが実行されフィードバックとレスポンスを返す
