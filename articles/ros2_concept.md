...
本記事では、ROS2を扱う上で理解必須の重要概念について解説します。
...
- 概念として、配布単位（package）、実行処理単位（executable/library）、クラス単位（node/executor等）をちゃんと区別して把握することが重要です。
  - 例えば、node自体はただのクラスであり、実行処理単位から呼ばれて初めて役割を果たします。が、事実上1executable=1nodeで作成することが多くある為にnodeを実行処理単位だと勘違いしやすいので要注意です。
- ros2を使用して作られるソフトウェアは、下記の構成をとります。
  - ソフトウェアは、packageを単位として配布される。開発者は複数のpackageを組み合わせて望むシステムを構成する
  - packageは、ユーザーもしくは他のpackageから実行可能な１以上の処理を提供する。
  - 処理は、executableもしくはlibraryの形式で提供される。executableもしくはlibraryは、複数の関数やクラスを実行することで所定の処理を行う。executableはユーザもしくは他のpackageから直接実行可能な処理を提供し、libraryは他のpackageから読み込まれて間接的に実行される処理を提供する。
...
- ROS2上で実行する処理を定義する基本構成単位となるクラス
- 標準node、カスタムnode
  - ros2には、標準node（NodeもしくはLifecycleNodeというクラス）が定義されている。独自の処理を持ったnodeを作るためにこれら標準nodeをそのまま使用してもよいし、標準nodeを継承したnode（＝カスタムnode）を作成してもよい。
  - 使い分け
    - 独自処理をnodeの中にカプセル化したいならカスタムnodeを作成する。
    - ライフサイクルイベントのハンドリングを行いたいならカスタムnodeを作成する。
    - どちらも不要であれば、NodeもしくはLifecycleNodeを直接用いる。
  ...
  - カスタムnodeを作成する場合のベストプラクティス
    ...
    - カスタムnodeを特定のexecutableに密結合させずに分離することで柔軟な運用が可能になる

### nodeを動かす手段

- nodeは必ずexecutor上で動く
  - rclcpp:spin(node)で実行した場合でも、内部でSingleThreadedExecutorが生成されその中でnodeが動いている
