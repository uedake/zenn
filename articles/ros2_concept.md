...
| グローバルROS引数 | ノードの初期化処理を制御する | 入力形式＝コマンドライン引数。保管形式＝`rcl_arguments_impl_s`構造体 | str |
| ローカルROS引数 | ノードの初期化処理を制御する | `rcl_arguments_impl_s`構造体 | str |
...
| グローバルROS引数 | 不要 | 自プロセスの値をグローバルデフォルトコンテキストから読みだせる | 不可 | executable実行時のコマンドライン引数として値を指定する |
